        ...
        <div class="space-y-4">
          ...
          <div class="rounded-2xl border border-neutral-800 bg-neutral-950 p-4">
            ...
            <div v-if="overview" class="space-y-3">
              ...
              >
                <div class="text-xs text-neutral-500">{{ key.replace(/_/g, ' ') | capitalize }}</div>
          ...
          <div v-if="personality || inventory" class="grid gap-4 md:grid-cols-2">
            ...
            <div v-if="inventory" class="rounded-2xl border border-neutral-800 bg-neutral-950 p-4">
              ...
              <ul class="list-inside list-disc text-sm text-neutral-200">
                <li v-for="(it, idx) in (Array.isArray(inventory) ? inventory : Object.values(inventory))" :key="idx">{{ it }}</li>
              </ul>
            </div>
          </div>
        </div>
...
<script lang="ts">
// Register a simple capitalize filter (for template usage)
import { defineNuxtPlugin } from '#app'
export default defineNuxtPlugin(() => {
  // noop, but filters may be implemented via globalProperties if needed
})
</script>

<style scoped>
/* small nicety: ensure prose shows correctly in dark bg */
