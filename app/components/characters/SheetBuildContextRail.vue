...
const props = withDefaults(defineProps<{
  activePanel?: BuildContextPanelKey | string
  choiceCount?: number
...
const emit = defineEmits<{
  (event: 'update-panel', value: BuildContextPanelKey): void
  (event: 'close'): void
}>()
...
const tabs = computed(() => [
  ...
  },
  {
    ...
    description: 'Pending level-up, skill, feat, spell, and setup choices.'
  },
  {
    key: 'inventory',
    label: 'Inventory',
...
const activePanelKey = computed<BuildContextPanelKey>(() => {
  const key = String(props.activePanel || 'setup')
  return tabs.value.some((tab) => tab.key === key) ? key as BuildContextPanelKey : 'setup'
})
...
function setPanel(value: BuildContextPanelKey) {
...
<template>
  ...
  >
    ...
    <main class="p-4">
      <div v-if="activePanelKey === 'setup'" class="grid gap-4">
        <slot name="setup" />
      </div>

      <div v-else-if="activePanelKey === 'choices'" class="grid gap-4">
        <slot name="choices" />
      </div>

      <div v-else-if="activePanelKey === 'inventory'" class="grid gap-4">
        <slot name="inventory" />
      </div>

      <div v-else-if="activePanelKey === 'spells'" class="grid gap-4">
        <slot name="spells" />
      </div>

      <div v-else-if="activePanelKey === 'features'" class="grid gap-4">
        <slot name="features" />
      </div>
    </main>
  </aside>
