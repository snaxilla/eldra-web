...
const showDm = computed(() => props.mode === 'run' || props.mode === 'build')
const showEdit = computed(() => props.mode === 'build' || props.mode === 'run')
</script>

<template>
  <aside class="flex h-full w-full flex-col border-l border-white/10 bg-[rgba(7,13,22,0.92)] backdrop-blur-md">
    <div class="flex items-center justify-between border-b border-white/10 px-5 py-4">
      <NuxtLink
        to="/"
        class="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-sm font-medium text-slate-100 transition hover:bg-white/[0.08]"
      >
        {{ worldName }}
      </NuxtLink>
    ...
    <div v-else class="flex flex-1 items-center justify-center p-8 text-center">
      <div>
        ...
        <div class="mt-3 text-lg text-slate-300">
          Select a pin on the map to inspect an article.
        </div>
      </div>
    </div>
  </aside>
