...
type BuildContextPanelKey = 'setup' | 'choices' | 'inventory' | 'spells' | 'features'

const buildContextPanel = ref<BuildContextPanelKey>('setup')
...
const buildContextOpen = computed(() =>
  mode.value === 'build' &&
  activeSheetTab.value === 'overview' &&
  !detailContextOpen.value
)

const sheetContextRailOpen = computed(() =>
  detailContextOpen.value || buildContextOpen.value
)

function normalizeBuildContextPanel(value: any): BuildContextPanelKey {
  const key = String(value || 'setup')
  return ['setup', 'choices', 'inventory', 'spells', 'features'].includes(key)
    ? key as BuildContextPanelKey
    : 'setup'
}

function setBuildContextPanel(value: any) {
  buildContextPanel.value = normalizeBuildContextPanel(value)
}

function closeSheetContextDetails() {
...
function openBuildContextPanel(value: any = 'setup') {
  closeSheetContextDetails()
  buildContextPanel.value = normalizeBuildContextPanel(value)
  ...
  if (activeSheetTab.value !== 'overview') {
    setSheetTab('overview')
  }
}

function closeBuildContextRail() {
  mode.value = 'play'
}
...
<template>
  ...
  >
            ...
            >
              <template #actions>
                <CharactersSheetActionCenter
                  ...
                  :open-item-drawer="openItemDrawer"
                  :open-spell-drawer="openSpellDrawer"
                  :open-build-panel="openBuildContextPanel"
    ...
    <CharactersSheetBuildContextRail
      v-if="buildContextOpen"
      data-sheet-build-context-rail
      :active-panel="buildContextPanel"
      :choice-count="levelSetupChoiceCount"
      :inventory-count="inventoryCount"
      :spell-count="selectedSpellCount"
      :feature-count="featureCount"
      :saving="sheetSaving || inventorySaving || spellSaving || choiceSaving"
      @update-panel="setBuildContextPanel"
      @close="closeBuildContextRail"
    >
      <template #setup>
        <CharactersSheetLevelManager
          :level="currentLevelNumber"
